...
class MapVC: UIViewController, UIGestureRecognizerDelegate {
    ...
    func addSwipe() {
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(animateViewDoun))
...
extension MapVC: MKMapViewDelegate {
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        let pinAnnotetion = MKPinAnnotationView(annotation: annotation, reuseIdentifier: "droppablePin")
        pinAnnotetion.pinTintColor = #colorLiteral(red: 0.9771530032, green: 0.7062081099, blue: 0.1748393774, alpha: 1)
        pinAnnotetion.animatesDrop = true
        return pinAnnotetion
    }
    ...
    @objc func dropPin(sender: UITapGestureRecognizer) {
        ...
        retrieveUrls(forAnnotation: annotation) { (finished) in
            if finished {
                self.retrieveImages(heandler: { (finished) in
                    if finished {
                        self.removeSpinner()
